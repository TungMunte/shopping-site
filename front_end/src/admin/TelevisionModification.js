...
function TelevisionModification() {
  ...
  useEffect(() => {
    async function getSmartphone() {
      const response = await axios.get(
        `http://localhost:8080/api/television/get/${id}`,
        {
          headers: {
  ...
  async function handleImage(event) {
    ...
    if (event.target.files && event.target.files[0]) {
      ...
      const res = await axios.put(
        `http://localhost:8080/api/television/image/put/${id}/${oldCode}/${newCode}`,
        data,
        {
          headers: {
            Authorization: `Bearer ${authContext.accessToken}`,
  ...
  async function handleSubmit(event) {
    try {
      ...
      const response = await axios.put(
        `http://localhost:8080/api/television/put/${id}`,
        {
          brand,
          technology,
          price,
              ...
              <div className="card-body p-5 text-center">
                {completed === false && (
                  <div className="mb-md-5 mt-md-4 pb-5">
                    ...
                    <div className="form-outline form-white mb-4">
                      <img
                        src={`http://localhost:8080/api/television/image/get/${imageCode}`}
                        style={{
                          width: "130px",
                          height: "130px",
                        }}
...
export default TelevisionModification;
