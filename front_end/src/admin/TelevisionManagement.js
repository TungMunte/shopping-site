...
function TelevisionManagement() {
  ...
  const handleDelete = async (event, id) => {
    ...
    const response = await axios.delete(
      `http://localhost:8080/api/television/delete/${id}`,
      {
        headers: {
      ...
      <div className="container">
        <div className="row justify-content-center">
          <div className="col-md-6 text-center mb-5">
            <h2 className="heading-section">Television</h2>
          </div>
        </div>
        <div className="row">
                  ...
                  {smartphones.map((smartphone) => {
                    return (
                      <tr>
                        <td>
                          <img
                            src={`http://localhost:8080/api/television/image/get/${smartphone.imageCode}`}
                            style={{
                              width: "130px",
                              height: "130px",
                            }}
